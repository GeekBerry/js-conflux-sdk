...
test('bigUInt', () => {
  ...
  expect(format.bigUInt(3.00)).toEqual(BigInt(3));
  expect(format.bigUInt('3.00')).toEqual(BigInt(3));
...
test('hexUInt', () => {
  ...
  expect(() => format.hexUInt(3.50)).toThrow('cannot be converted to a BigInt');
  expect(() => format.hexUInt(-0.5)).toThrow('cannot be converted to a BigInt');
  expect(() => format.hexUInt(-1)).toThrow('not match bigUInt');
  expect(() => format.hexUInt(null)).toThrow('Cannot convert null to a BigInt');
});
